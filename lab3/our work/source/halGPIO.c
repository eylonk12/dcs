...
// Arrays for information of the delay needed to generate PWM with frequencies
// that match the notes on the songs.

int song1[55] = { 893, 669, 669, 669, 893, 795, 795, 893, 530, 530, 595, 595, 669, 893, 669, 669,
...
int song2[48] = { 1002, 751, 595, 1002, 751, 595, 595, 669, 669, 669, 669, 595, 562, 562, 562, 500,
...
int song3[41] = { 795, 669, 795, 795, 669, 795, 500, 530, 595, 669, 751, 893, 751, 595, 669, 669,
                        751, 795, 893, 1002, 795, 669, 795, 795, 669, 795, 500, 530, 595, 669, 751, 893,
                        751, 595, 669, 669, 751, 795, 893, 1002, 0};

// Arrays for information of the delay needed to generate duty cycle of 50%.
...
// Strings to screen.
char starting_string1[15] = {"Recording Start"};
char starting_string2[15] = {"Press a key"};
char output_string[16] = {"Record Length:  "};
char audio_menu_1[16] = {"Now choose song:"};
char audio_menu_2[16] = {"Press PB2 scroll"};
char audio_menu_3[14] = {"L1-Hadod Moshe"};
char audio_menu_4[16] = {"L2-Haoto Shelano"};
char audio_menu_5[16] = {"L3-Yadaim Lemala"};
char audio_menu_6[16] = {"L4-Recorder song"};
// variables
volatile int* chosen_song; 
volatile int* chosen_song_dc; 
...
void display_menu(void){
        ...
        lcd_puts(starting_string1);
        lcd_cmd(0xC0);// New line.
        lcd_puts(starting_string2);
  }
...
void choose_song_from_menu(){
  ...
  while(display_screen <4) {
    ...
    case 0:
      lcd_clear();
      lcd_puts(audio_menu_1);
      lcd_new_line;
      lcd_puts(audio_menu_2);
      enterLPM(lpm_mode);    // wait for response.
      break;
    case 1:
      lcd_clear();
      lcd_puts(audio_menu_3);
      lcd_new_line;
      lcd_puts(audio_menu_4);
      enterLPM(lpm_mode);    // wait for response.
      break;
    case 2:
      lcd_clear();
      lcd_puts(audio_menu_5);
      lcd_new_line;
      lcd_puts(audio_menu_6);
      enterLPM(lpm_mode);    // wait for response.
      break;
    }
  }
...
//---------------------------------------------------------------------
	if(PBsArrIntPend & PB0){
	  state = state1;
          Disnable_TimerB_PWM();   // shut down the DMA.
